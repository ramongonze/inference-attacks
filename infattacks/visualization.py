...
class VisualizeRisk:
    ...
    def _build_graph(self, x, y, show_graph, save_graph, format, attack_type, sensitive=None):
        ...
        plt.xticks(range(1,self.attack.result_comb_qids_reid["num_qids"].max()+1))
    ...
    def plot_hist(self, histogram_counts, title, hist_bin_size=5, show_hist=True, save_hist=None, format=None):
        ...
        Parameters:
            histogram_counts (list or np.ndarray): List or array containing the counts for each bin of the histogram. It's generated by `Attack.post_reid, `Attack.post_ai`, `Attack.post_comb_qids_reid` `Attack.post_comb_qids_ai` methods.
